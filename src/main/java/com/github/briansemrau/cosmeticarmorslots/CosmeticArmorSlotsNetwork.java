...
public class CosmeticArmorSlotsNetwork {
    ...
    public static CustomPayloadS2CPacket createEquipCosmeticArmorPacket(int entityId, EquipmentSlot slot, ItemStack stack, boolean visible) {
        ...
        return new CustomPayloadS2CPacket(EQUIP_COSMETIC_ARMOR_PACKET, buf);
    }

    public static CustomPayloadC2SPacket createCosmeticSlotVisibilityUpdatePacket(EquipmentSlot slot, boolean visible) {
        PacketByteBuf buf = new PacketByteBuf(Unpooled.buffer());
        buf.writeEnumConstant(slot);
        buf.writeBoolean(visible);
        return new CustomPayloadC2SPacket(VISIBILITY_UPDATE_PACKET, buf);
    }
